 ...
 *
 * 工行账单格式特征：
 * - CSV格式：交易日期,摘要,金额,余额,对方账户,交易渠道
 * - 日期格式：2024-06-15 或 20240615
 * - 金额正负号表示收支
 * - 摘要包含交易类型和商户信息
 */
...
/** 收入关键词 */
const INCOME_KEYWORDS = ['工资', '汇入', '转入', '存入', '利息', '收益', '退款', '代发', '收入'];

/** 支出关键词 */
const EXPENSE_KEYWORDS = ['消费', '汇出', '转出', '取款', '缴费', '支付', '扣款', '支出', '网购'];

@Injectable()
export class IcbcParser implements IBillParser {
  private readonly logger = new Logger(IcbcParser.name);
  readonly platform = 'icbc';
  ...
  private parseCSV(text: string): ParseResult {
    ...
    for (let i = headerIndex + 1; i < lines.length; i++) {
      ...
      try {
        ...
        const dateStr = (cols[ICBC_CSV_COLUMNS.date] || '').trim();
        const summary = (cols[ICBC_CSV_COLUMNS.summary] || '').trim();
        const amountStr = (cols[ICBC_CSV_COLUMNS.amount] || '').trim();
        ...
        const counterparty = (cols[ICBC_CSV_COLUMNS.counterparty] || '').trim();
        const merchant = counterparty || this.extractMerchant(summary);
        const channel = (cols[ICBC_CSV_COLUMNS.channel] || '').trim() || null;

        transactions.push({
          date,
          amount: Math.abs(amount),
          type,
          description: summary || '工商银行交易',
          merchant: merchant || null,
          transactionNo: null,
          paymentMethod: channel || '工商银行',
          status: null,
          raw: line,
        });
      } catch (err) {
    ...
    this.logger.log(`工商银行CSV解析完成: ${transactions.length}条交易, 跳过${skippedCount}条`);
    return { transactions, skippedCount, errors };
  }
  ...
  private parseText(text: string): ParseResult {
    ...
    this.logger.log(`工商银行文本解析完成: ${transactions.length}条交易, 跳过${skippedCount}条`);
    return { transactions, skippedCount, errors };
  }
